...
public class ServicioGestionProyecto {
    ...
    public void asignarIntegranteAProyecto(Long proyectoId, Long integranteId) {
        ProyectoInvestigacion proyecto = proyectoRepo.findById(proyectoId).orElseThrow(() -> new RunTimeException("Proyecto no encontrado"));
        IntegranteProyecto integrante = integranteRepo.findById(integranteId).orElseThrow(() -> new RunTimeException("Integrante no encontrado"));
